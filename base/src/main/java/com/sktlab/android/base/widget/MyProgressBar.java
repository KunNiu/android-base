package com.sktlab.android.base.widget;
...
public class MyProgressBar extends View {
    ...
    MyProgressBarTextGenerator mMyProgressBarTextGenerator;
    ...
    private Runnable mNotifyProgressChangeAction = new Runnable() {
        ...
        public void run() {
            if (mOnProgressChangeListener != null) {
                mOnProgressChangeListener.onProgressChange(MyProgressBar.this, mValue, mMaxValue);
            }
        }
    };
    ...
    public void setup(Context context, AttributeSet attrs) {
        ...
        mType = array.getInt(R.styleable.MyProgressBar_type, TYPE_RECT);
        mProgressColor = array.getColor(R.styleable.MyProgressBar_progress_color, DEFAULT_PROGRESS_COLOR);
        mBackgroundColor = array.getColor(R.styleable.MyProgressBar_background_color, DEFAULT_BACKGROUND_COLOR);

        mMaxValue = array.getInt(R.styleable.MyProgressBar_max_value, 100);
        mValue = array.getInt(R.styleable.MyProgressBar_value, 0);

        mRoundCap = array.getBoolean(R.styleable.MyProgressBar_stroke_round_cap, false);

        mTextSize = DEFAULT_TEXT_SIZE;
        if (array.hasValue(R.styleable.MyProgressBar_android_textSize)) {
            mTextSize = array.getDimensionPixelSize(R.styleable.MyProgressBar_android_textSize, DEFAULT_TEXT_SIZE);
        }
        mTextColor = DEFAULT_TEXT_COLOR;
        if (array.hasValue(R.styleable.MyProgressBar_android_textColor)) {
            mTextColor = array.getColor(R.styleable.MyProgressBar_android_textColor, DEFAULT_TEXT_COLOR);
        }

        if (mType == TYPE_CIRCLE) {
            mStrokeWidth = array.getDimensionPixelSize(R.styleable.MyProgressBar_stroke_width, (int) Utils.dpToPx(context, 40));
        }
        array.recycle();
     ...
     * @see #setMyProgressBarTextGenerator(MyProgressBarTextGenerator)
     */
    ...
    /**
     * 通过 {@link MyProgressBarTextGenerator} 设置进度文案
     */
    public void setMyProgressBarTextGenerator(MyProgressBarTextGenerator MyProgressBarTextGenerator) {
        mMyProgressBarTextGenerator = MyProgressBarTextGenerator;
    }

    public MyProgressBarTextGenerator getMyProgressBarTextGenerator() {
        return mMyProgressBarTextGenerator;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mMyProgressBarTextGenerator != null) {
            mText = mMyProgressBarTextGenerator.generateText(this, mValue, mMaxValue);
        }
    ...
    public interface MyProgressBarTextGenerator {
        /**
         * 设置进度文案, {@link MyProgressBar} 会在进度更新时调用该方法获取要显示的文案
         *
         * @param value    当前进度值
         * @param maxValue 最大进度值
         * @return 进度文案
         */
        String generateText(MyProgressBar progressBar, int value, int maxValue);
    }

    public interface OnProgressChangeListener {
        void onProgressChange(MyProgressBar progressBar, int currentValue, int maxValue);
    }
}
